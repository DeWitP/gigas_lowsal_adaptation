#!/usr/bin/env Rscript
# rm(list = ls())
setwd("/scratch/05301/dewitp/dynamo/R/CADCXH")
...
OBS_REF <- read.delim("embryos14_CADCXH_REF.txt", header=TRUE)
OBS_ALT <- read.delim("embryos14_CADCXH_ALT.txt", header=TRUE)
...
TOTAL <- data.frame(matrix(ncol = 39, nrow = 748486))
...
Total_G_matrix <- data.frame(matrix(ncol = 39, nrow = 226))
Het_G_matrix <- data.frame(matrix(ncol = 39, nrow = 226))
df_matrix <- data.frame(matrix(ncol = 39, nrow = 226))
