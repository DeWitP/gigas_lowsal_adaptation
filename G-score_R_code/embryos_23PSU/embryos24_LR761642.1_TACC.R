#!/usr/bin/env Rscript
# rm(list = ls())
setwd("/scratch/05301/dewitp/dynamo/R/embryos24/LR761642")
...
load("expected_props_LR761642.1.Rdata")

#Reading in the observed counts (where "NA" has been replaced with zeroes):
OBS_REF <- read.delim("embryos24_LR761642.1_REF.txt", header=TRUE)
OBS_ALT <- read.delim("embryos24_LR761642.1_ALT.txt", header=TRUE)
...
TOTAL <- data.frame(matrix(ncol = 41, nrow = 1052510))
...
# max(OBS_REF$POS)
## Max position is: 37069116 (37.07 MB)

# This means that the G-matrix should have 371 100-KB blocks, to cover the whole chromosome.
...
Total_G_matrix <- data.frame(matrix(ncol = 41, nrow = 371))
Het_G_matrix <- data.frame(matrix(ncol = 41, nrow = 371))
df_matrix <- data.frame(matrix(ncol = 41, nrow = 371))
