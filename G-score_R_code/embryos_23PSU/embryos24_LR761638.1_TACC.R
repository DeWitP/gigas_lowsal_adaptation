#!/usr/bin/env Rscript
# rm(list = ls())
setwd("/scratch/05301/dewitp/dynamo/R/embryos24/LR761638")
...
load("expected_props_LR761638.1.Rdata")

#Reading in the observed counts (where "NA" has been replaced with zeroes):
OBS_REF <- read.delim("embryos24_LR761638.1_REF.txt", header=TRUE)
OBS_ALT <- read.delim("embryos24_LR761638.1_ALT.txt", header=TRUE)
...
TOTAL <- data.frame(matrix(ncol = 41, nrow = 1602477))
...
# max(OBS_REF$POS)
## Max position is: 73498724 (73.50 MB)

# This means that the G-matrix should have 735 100-KB blocks, to cover the whole chromosome.
...
Total_G_matrix <- data.frame(matrix(ncol = 41, nrow = 735))
Het_G_matrix <- data.frame(matrix(ncol = 41, nrow = 735))
df_matrix <- data.frame(matrix(ncol = 41, nrow = 735))
