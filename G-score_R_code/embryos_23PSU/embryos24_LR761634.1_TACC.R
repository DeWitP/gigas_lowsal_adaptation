#!/usr/bin/env Rscript
setwd("/scratch/05301/dewitp/dynamo/R/embryos24/LR761634")
...
load("expected_props_LR761634.1.Rdata")

#Reading in the observed counts (where "NA" has been replaced with zeroes):
OBS_REF <- read.delim("embryos24_LR761634.1_REF.txt", header=TRUE)
OBS_ALT <- read.delim("embryos24_LR761634.1_ALT.txt", header=TRUE)
...
TOTAL <- data.frame(matrix(ncol = 41, nrow = 2209213))
...
## Max position is: 55761714 (55.76 MB)

# This means that the G-matrix should have 558 100-KB blocks, to cover the whole chromosome.
...
Total_G_matrix <- data.frame(matrix(ncol = 41, nrow = 558))
Het_G_matrix <- data.frame(matrix(ncol = 41, nrow = 558))
df_matrix <- data.frame(matrix(ncol = 41, nrow = 558))
